...
void joyCallback(const sensor_msgs::Joy::ConstPtr &joy) {
  ...
  if (joy->buttons[reset_button] == 1 &&
      (!is_scan_ptu_requested && !is_gas_scan_requested)) {
    is_reset_ptu_requested = true;
    ROS_INFO("Here");
  } else if (joy->buttons[scan_button] == 1 &&
             (!is_scan_ptu_requested && !is_gas_scan_requested)) {
    is_scan_ptu_requested = true;
  }

  else if (joy->buttons[gas_scan_button] == 1 &&
           (!is_scan_ptu_requested && !is_gas_scan_requested)) {
    is_gas_scan_requested = true;
  } else {

    if ((lock_trigger && joy->axes[lock_button] > 0 && is_lock_ptu_requested) ||
        (!lock_trigger && joy->buttons[lock_button] == 0 &&
         is_lock_ptu_requested))
      is_lock_ptu_requested = false;
    else if ((lock_trigger && joy->axes[lock_button] <= 0) ||
             (!lock_trigger && joy->buttons[lock_button] == 1)) {
...
// 	Service callback
//-------------------------------------------------------
bool sweepRequestCallback(ptu_control::commandSweep::Request &req,
                          ptu_control::commandSweep::Response &resp) {
...
int main(int argc, char **argv) {
  ...
  ros::Publisher state_pub =
      n.advertise<std_msgs::Int16>("/ptu_control/state", 1000);
  ros::ServiceServer service =
      n.advertiseService("/ptu_control/sweep", sweepRequestCallback);
  ...
  while (ros::ok()) {
    ...
    if (joystick_control_enabled && !service_running) {
      ...
      if (is_reset_ptu_requested) {
        ...
        if (enable_environment_scan) {
          performSceneScan(state_pub, scan_min_tilt_deg, scan_max_tilt_deg,
                           client, clientStatus);
        }
        sleep(1);
        performGasScan(state_pub, scan_min_tilt_deg, scan_max_tilt_deg,
                       scan_min_pan_deg, scan_max_pan_deg, n_pan_joy,
                       n_tilt_joy, samp_delay_joy, client, clientStatus);
        sleep(1);
        resetPTUPosition(state_pub, client, clientStatus);
        is_gas_scan_requested = false;
      }
        ...
        // ROS_INFO("--- PTU locked, position
        // %f,%f",target_pan_deg,target_tilt_deg);

        if (ptu_horizontal_request > 0) {
          target_pan_deg = target_pan_deg + interp(angle_increments_deg,
                                                   fabs(ptu_horizontal_request),
                                                   N_DELTA_DEG);
        ...
        if (ptu_vertical_request > 0) {
          target_tilt_deg =
              target_tilt_deg - interp(angle_increments_deg,
                                       fabs(ptu_vertical_request), N_DELTA_DEG);
          // target_pan_deg=deg2rad(target_pan_deg);
          // target_tilt_deg=deg2rad(target_tilt_deg);
        ...
        // ROS_INFO("Requested PTU LOCATION
        // %f,%f",target_pan_deg,target_tilt_deg);
      }
    ...
    // Service requestes//
      ...
      if (enable_environment_scan) {
        performSceneScan(state_pub, min_tilt_srv, max_tilt_srv, client,
                         clientStatus);
      }

      sleep(0.5);
      performGasScan(state_pub, min_tilt_srv, max_tilt_srv, min_pan_srv,
                     max_pan_srv, n_pan_srv, n_tilt_srv, samp_delay_srv, client,
                     clientStatus);
      sleep(0.5);
      resetPTUPosition(state_pub, client, clientStatus);
      service_running = false;
...
void retrieveLaunchParameters(ros::NodeHandle n) {
  ...
  // AZIF: Set a parameter to enable/disable the environment scan
...
// 	Scene Scan
//-------------------------------------------------------
void performSceneScan(ros::Publisher state_pub, double min_tilt_deg,
                      double max_tilt_deg, ros::ServiceClient client,
                      ros::ServiceClient clientStatus) {
  amtec::SetPosition srv;
  amtec::GetStatus srvState;
  ...
  do {
    ...
    if (clientStatus.call(srvState)) {
      retry_count++;
      ROS_INFO("Moving");
      if (fabs(rad2deg(srvState.response.position_tilt) - max_tilt_deg) <=
              0.8 &&
          fabs(rad2deg(srvState.response.position_pan)) <= 0.8) {
        pos_reached = true;
      }
...
void performGasScan(ros::Publisher state_pub, double min_tilt_deg,
                    double max_tilt_deg, double min_pan_deg, double max_pan_deg,
                    int n_pan, int n_tilt, double samp_delay,
                    ros::ServiceClient client,
                    ros::ServiceClient clientStatus) {
  ...
  do {
    ...
    if (clientStatus.call(srvState)) {
      retry_count++;
      // ROS_INFO("Moving");
      if (fabs(rad2deg(srvState.response.position_tilt) - min_tilt_deg) <=
              0.8 &&
          fabs(rad2deg(srvState.response.position_pan) - min_pan_deg) <= 0.8) {
        pos_reached = true;
      }
  ...
  do {
    if (current_movement == TO_RIGHT) {
      ...
      do {
        clientStatus.call(srvState);
        sleep(0.1);
      } while (fabs(srvState.response.velocity_tilt) > 0.0 ||
               fabs(srvState.response.velocity_pan) > 0.0);
...
// 	PTU reset position
//-------------------------------------------------------
void resetPTUPosition(ros::Publisher state_pub, ros::ServiceClient client,
                      ros::ServiceClient clientStatus) {
  amtec::SetPosition srv;
  amtec::GetStatus srvState;
  ...
  do {
    ...
    if (clientStatus.call(srvState)) {
      // ROS_INFO("Gotcha
      // %f,%f",rad2deg(srvState.response.position_tilt),rad2deg(srvState.response.position_pan));
      retry_count++;
      if (fabs(rad2deg(srvState.response.position_tilt)) <= 0.8 &&
          fabs(rad2deg(srvState.response.position_pan)) <= 0.8) {
        pos_reached = true;
      } else if (retry_count > 20) {
        ROS_INFO("Warning: PTU not responding. Retrying");
